...
from collector import BigQueryCollector
import config
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        prog = 'BigQuery-exporter',
        description = 'Python BigQuery Prometheus Exporter'
    )
...
def main():
    """Register the BigQuery collector and start a HTTP server."""
    ...
    try:
        ...
        # Register the BigQuery collector (its 'collect' method is called on registration)
        REGISTRY.register(BigQueryCollector())
    except BaseException as exc:
        logger.exception('There was an error starting the BigQuery exporter')
        sys.exit(1)
